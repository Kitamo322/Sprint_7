...
def create_courier():
    ...
    create_courier = requests.post(f'{Url.MAIN_URL}{Url.CREATE_COURIER}', json=create_courier_body)
    login_courier = requests.post(f'{Url.MAIN_URL}{Url.COURIER_LOGIN}', json=login_courier_body)
    yield (create_courier_body, create_courier.status_code, create_courier.json(), login_courier.json()['id'],
           login_courier.status_code, login, password)
